...
function handleRequest(request, response, pool) {
  ...
  pool.getConnection(function(err, connection) {
    ...
    connection.execute(
      `SELECT fio as ПІБ, login_ad, short_fio, stuff, department
       FROM tmp_ad_users
       WHERE department_ou = :id`,
      ['ITMC'], // bind variable value
      function(err, result) {
        if (err) {
...
function htmlHeader(response, title, caption) {
  ...
  response.write("<style>" +
    "body {background:#FFFFFF;color:#000000;font-family:Arial,sans-serif;margin:40px;padding:10px;font-size:14px;text-align:center;}" +
    "h1 {margin:0px;margin-bottom:12px;background:#61af13;text-align:center;color:#FFFFFF;font-size:28px;}" +
